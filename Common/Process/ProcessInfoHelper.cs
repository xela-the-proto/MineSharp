...
public class ProcessInfoHelper
{
    ...
    /// <param name="workDir">
    ///     When the <c>useShellExecute</c> property is false, gets or sets the working directory for the process to be
    ///     started.
    ///     When <c>useShellExecute</c> is true, gets or sets the directory that contains the process to be started.
    /// </param>
    ...
    public static System.Diagnostics.Process BuildStarterProcess(string command, List<string> flags
        , string workDir, bool hookIntoStdErr = true, bool hookIntoStdOut = true, bool hookIntoStdIn = true,
        bool useShellExecute = true)
    {
        var args = "";
        for (var i = 0; i < flags.Count; i++) args += flags[i] + " ";
    ...
    protected static string BuildArguments(List<string> flags)
    {
        ...
        foreach (var flag in flags) builtArgs += flag + " ";
